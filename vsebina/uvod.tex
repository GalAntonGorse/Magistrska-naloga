...
Eden izmed načinov, na katerega lahko motiviramo povsem pozitivne preslikave, pride iz nekomutativne verjetnosti.
...
kot na par $(\mathfrak{A}, \E)$,
kjer je $\mathfrak{A}$ $C^*$-algebra in $\E: \mathfrak{A} \to \C$ pozitiven funkcional,
za katerega velja $\E (1) = 1$ (takšnim funkcionalom pravimo \emph{stanja}).
...
(več o temi v \cite{nicaspeicher2006}). Oglejmo si Markovsko jedro za merljiva prostora $(X, \mathcal{F})$ in $(Y, \mathcal{G})$,
...
ali pa v \emph{kvantni teoriji informacij}, kjer so znane kot \emph{kvantni kanali} (\cite{nielsenchuang2010}).
...
(\cite{connes2021}), najdemo jih pa tudi v kvantni teoriji informacij kot \emph{kvantne grafe} (\cite{weaver2021}, \cite{duanseveriniwinter2013}).
...
teorijo povsem omejenih preslikav in operatorskih prostorov, ki jo 
lahko razumemo kot tesen analog teorije povsem pozitivnih preslikav in operatorskih sistemov.
V tem magistrskem delu se s tem ne bomo ukvarjali; celovit pregled lahko bralec najde v \cite{paulsen2002} ali
\cite{pisier2003}.
...
v katerem sta avtorja dokazala, da ima vsak operatorski sistem abstraktno karakterizacijo, ki je torej neodvisna od konkretne $C^*$-algebre,
...
Ekvivalentno vprašanje lahko postavimo za konkreten operatorski sistem $\mathcal{S} \subseteq C^*(\mathcal{S})$: ali taka obstaja minimalna $C^*$-algebra 
$\mathfrak{A}$ in enotska povsem izometrija $i: \mathcal{S} \to \mathfrak{A}$, da je $\mathfrak{A} = C^*(i(\mathcal{S}))$?
...
V ta namen je vpeljal nekomutativen analog geometrijskega pojma roba Šilova, ki smo ga omenili na začetku uvoda.
...
(več o tem v \cite{gamelin1969}). Da lahko vpeljemo nekomutativen rob Šilova,
...
Ker so zaprte podmnožice $X$ v bijektivni korespondenci z zaprtimi ideali v $C(X)$, lahko vpeljemo 
...
Arveson je poleg roba Šilova vpeljal še nekomutativen analog tesno povezanega geometrijskega pojma: Choquetovega roba.
To je pojem iz Choquetove teorije, ki se okvirno ukvarja s problemi naslednjega tipa:
...
Za splošne (ne nujno separabilne) operatorske sisteme sta trditev dokazala Davidson in Kennedy leta 2015 v članku \cite{davidson2015},
