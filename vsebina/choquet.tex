...
\section{Nekomutativen Choquetov rob}
...
Dokazali bomo, da za vsak operatorski sistem $\mathcal{S}$ obstaja dovolj robnih upodobitev, da \emph{povsem normirajo} $\mathcal{S}$.
Pri tem sledimo članku Davidsona in Kennedyja \cite{davidson2015},
v katerem avtorja posplošita argumente Arveson-Dritschell-McCollougha izreka.
...
\begin{opomba}
    Pojem Choquetovega robu izhaja iz Choquetove teorije, ki se okvirno ukvarja s problemi naslednjega tipa:
    ...
    Pomemben rezultat v tej smeri je Choquetov izrek (DOPOLNI), ki na to vprašanje odgovori pritrdilno, v kolikor je $E$ metrizabilen prostor.
    Za več o tej temi glej (DOPOLNI).
\end{opomba}
...
\begin{zgled}
    ...
    Znano dejstvo je (DOPOLNI), da za vsako realno harmonično funkcijo $g$ na $\D$, ki je zvezna na $\overline{\D}$, velja Poissonova reprezentacijska formula
    ...
    Opomnimo, da rob Šilova in Choqueta za operatorski sistem $\mathcal{S}$ sovpadata. 
...
Če želimo pojem Choquetovega robu posplošiti na nekomutativen operatorski sistem $\mathcal{S} \subseteq C^*(\mathcal{S})$,
moramo najti primeren nekomutativen analog preslikavam $\ev_x :C(X) \to \C$, kjer je $x \in X$.
...
Po standardnem izreku iz funkcionalne analize (DOPOLNI) je slednji pogoj ekvivalenten $\pi(\mathfrak{A}) ' = \C \cdot I$.
...
\begin{trditev}
    Vsaka nerazcepna upodobitev $C^*$-algebre $C(X)$ je oblike $\ev_x : C(X) \to \C$
    za neko točko $x \in X$.
\end{trditev}

\begin{dokaz}
    Naj bo $\rho: C(X) \to \bh$ nerazcepna upodobitev, torej $\rho(C(X))' = \C \cdot I$.
    Ker je $C(X)$ komutativna algebra, je tudi $\rho(C(X))$ komutativna algebra in v posebnem velja 
    $\rho(C(X)) \subseteq \rho(C(X))' = \C \cdot I$. To pomeni, da je vsak podprostor v $\mathcal{H}$
    ...
    Naprej dokazujemo, da je njeno jedro $\ker \rho$ maksimalen ideal v $C(X)$. Vzemimo poljuben ideal $\ker \rho \subsetneqq I \subseteq C(X)$,
    ...
    tako da je $\ker \rho = I(\{x\})$. Potem je za poljubno funkcijo $f \in C(X)$
...
\begin{zgled}
    ...
    in po Banach-Alaoglujevem izreku (DOPOLNI) kompaktna v šibki-$*$ topologiji.
    Po Krein-Milmanovem izreku (DOPOLNI) ima zato neprazno množico ekstremnih točk, ki jo označujemo s $\ext S(\mathcal{S})$.
    ...
    Denimo najprej, da stanje $\omega$ ni čisto, torej obstajata $\omega_1, \omega_2 \in S(\mathcal{S})$, ki nista enaka $\omega$ in za katera velja
...
\begin{trditev}
    ...
    njena minimalna Stinespringove dilatacija $(\pi, V, \mathcal{K})$ nerazcepno upodobitev $\pi$.
\end{trditev}
...
\begin{dokaz}
    ...
    kar pa je ekvivalentno temu, da je $\pi$ nerazcepna (glej DOPOLNI).
\end{dokaz}
...
\begin{dokaz}
    Denimo, da $\rho$ ni nerazcepna, torej obstaja nek $(0) \lneqq \mathcal{K} \lneqq \mathcal{H}$,
    ki reducira $\rho$. Naj bo $P \in \bh$ ortogonalna projekcija na $\mathcal{K}$, torej $P \in \rho(C^* (\mathcal{S}))'$.
    Potem je $\chi := P \rho$ povsem pozitivna preslikava in $0 \leq \chi\big|_{\mathcal{S}} \leq \varphi$.
    Ker pa $\chi (1) = P$ ni skalar, $\varphi$ ne more biti čista preslikava.
\end{dokaz}

Naslednji izrek je analog Arveson-Dritschell-McColloughovega izreka za čiste preslikave.
...
\begin{dokaz}
    Obstoj in enoličnost enotske povsem pozitivne preslikave $\varphi_\infty$ že imamo po lemi \ref{lem:2.1}.
    Dokazati moramo le, da je $\varphi_\infty$ tudi čista preslikava.
    Vzemimo poljubno preslikavo $\psi \in \PP (\mathcal{S}, \mathcal{B}(\mathcal{H}_\infty))$, 
...
\begin{dokaz}
    ...
    in $(\alpha_1^{\frac{1}{2}}, \alpha_2 ^{\frac{1}{2}})$. Ker sta oba tudi enotska, sta enaka in imamo $\lambda_1 = \alpha_1$ ter $\lambda_2 = \alpha_2$.
...
Sedaj lahko dokažemo izrek \ref{izr:2.2}. Pri tem skoraj popolnoma ponovimo dokaz izreka \ref{izr:2.2},
...
\begin{dokaz}[Dokaz izreka \ref{izr:2.2}]
    ...
    \begin{enumerate}
        ...
        Označimo z $(a_0, x_0)$ minimum dobre urejenosti. Po trditvi \ref{trd:2.3} obstaja čista dilatacija $\varphi_0: \mathcal{S} \to \mathcal{B}(\mathcal{H}_0)$,
        ...
        za vsak $\nu \preceq \mu$.
        Tedaj po trditvi \ref{trd:2.3} obstaja čista dilatacija $\varphi_\lambda \succeq \varphi_\nu$,
...
Z Davidson-Kennedyjevim izrekom lahko dokažemo sedaj glavni izrek tega poglavja.
...
\begin{dokaz}
    ...
    Po lemi (DOPOLNI) obstaja upodobitev $\rho : C^* (\mathcal{S}) \to \mathcal{B}(\mathcal{K}_0)$, tako da je $\psi'$ unitarno ekvivalentna $\rho ^{(2n)}$,
...
\begin{posledica}
    Naj bo $\mathcal{S}$ operatorski sistem in $\pi: C^*(\mathcal{S}) \to \bh$ direktna vsota vseh robnih upodobitev na $\mathcal{S}$.
    Potem je $\pi \big|_{\mathcal{S}}: \mathcal{S} \to \pi(C^*(\mathcal{S}))$ $C^*$-ogrinjača $\mathcal{S}$ in 
    $$\ker \pi = \bigcap \{\ker \rho\ |\ \text{$\rho$ robna upodobitev za $\mathcal{S}$}\}$$
...
\begin{dokaz}
    ...
    \begin{align*}
        I(Y) = \bigcap \{\ker \varphi\ |\ \text{$\varphi$ robna upodobitev za $\mathcal{S}$}\} = \bigcap_{x \in Z} I(\{x\}) = I(\overline{Z}).
    \end{align*}
    Preostane le še utemeljiti zadnjo enakost. Očitno je $I(\overline{Z}) \subseteq \bigcap_{x \in Z} I(\{x\})$.
    Obratno mora vsaka funkcija $f \in \bigcap_{x \in Z} I(\{x\})$ po zveznosti biti ničelna na zaprtju $\overline{Z}$,
    od koder dobimo $f \in I(\overline{Z})$.
\end{dokaz}

\begin{zgled}
    Vrnimo se k zgledu \ref{zgl:2.2} in poiščimo vse robne upodobitve za operatorski sistem 
    $\mathcal{T}^{(n)} \subseteq M_n (\C)$. Po lemi (DOPOLNI) so vse nerazcepne upodobitve
...
\begin{zgled}
    ...
    \begin{equation*}
        ...
        \end{tikzpicture}    
    \end{equation*}
    pri čemer je $\overline{\rho}: \frac{C^*({S})}{\mathcal{K}(H^2)} \to \bh$
    nerazcepna upodobitev za $\frac{C^*({S})}{\mathcal{K}(H^2)}$.
    
    V zgledu \ref{zgl:2.3} smo dokazali, da velja $\frac{C^*({S})}{\mathcal{K}(H^2)} \cong C(\T)$,
    zato je $\mathcal{H} = \C$ in $\overline{\rho} = \ev_\lambda: C(\T) \to \C$ za nek $\lambda \in \T$.
    S tem smo dokazali, da so vse robne upodobitve oblike 
    $$\rho_\lambda: C^*(S) \to \C,\quad \rho_\lambda(T_f + K) = f(\lambda).$$ 
    Dokazati moramo še, da so $\rho_\lambda$ res robne upodobitve za $\mathcal{T}(C(\T))$.
    
    Vzemimo poljuben $\lambda \in \T$. Po izreku \ref{izr:2.4} mora obstajati neka robna upodobitev $\rho$, tako da je
    $$\| \rho(T_{z + \lambda})\| = \| z + \lambda\| = 2.$$
