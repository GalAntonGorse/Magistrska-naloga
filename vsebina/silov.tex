\section{Abstraktni operatorski sistemi}
...
Choi in Effros sta pokazala (\cite{ruan1988}), da tudi operatorski sistemi, ki jih definiramo kot konkretne prostore operatorjev v $C^*$-algebri, 
premorejo abstraktno karakterizacijo. To nam omogoča, da lahko na operatorski sisteme gledamo kot na abstrakten vektorski prostor
...
$\mathcal{S}_h := \{s \in \mathcal{S}\ |\ s = s^*\}$.

Tudi v abstraktnem operatorskem sistemu želimo imeti ustrezne elemente, ki imajo vlogo pozitivnih elementov.
Zato mora za vsak $n \in \N$ obstajati konveksen stožec $\mathcal{C}_n \subseteq M_n (\mathcal{S})_h$, tako da velja $\mathcal{C}_n \cap -\mathcal{C}_n = \{0\}$. 
...
Element $e \in \mathcal{S}_h$ je \emph{enota za urejenost} v $\mathcal{S}$, 
če za vsak $a \in \mathcal{S}_h$ obstaja pozitivno realno število $r > 0$, tako da je 
...
V primeru, ko je $I_n $ arhimedska za vsak $n \in \N$, je $e$ \emph{arhimedska enota za matrično urejenost} za $\mathcal{S}$.
...
$se + a = (s - r)e + (re + a) \in \mathcal{C}_1 $. Če je $a \in \mathcal{S}_h$, potem obstaja dovolj velik $r > 0$,
tako da je $re \pm a \in \mathcal{C}_1$ in posledično 
$$a = \frac{re + a}{2} - \frac{re - a}{2} \in \mathcal{C}_1 - \mathcal{C}_1.$$
Od tod sledi, da je $\mathcal{S}_h = \mathcal{C}_1 - \mathcal{C}_1$.
...
\begin{dokaz}
    \begin{enumerate}
        ...
        Najprej dokažimo, da je $a \in \mathcal{S}_h$. Ker je $\lim_{n \to \infty} \| a - a_n\|_1 = 0$,
...
Vsak konkreten operatorski sistem $\mathcal{S} \subseteq \mathfrak{A}$ je na kanoničen način abstrakten operatorski sistem:
involucija $*$ je inducirana s $C^*$-algebro, konveksni stožci pa so $\mathcal{C}_n = M_n (\mathcal{S})_+$.
Dokažimo še, da je $1 \in \mathcal{S}$ arhimedska enota za matrično urejenost.
Brez škode za splošnost naj bo $\mathfrak{A} = \bh$ za nek Hilbertov prostor $\mathcal{H}$.
...
Denimo, da velja $r \cdot I_n + A \geq 0$ za vse $r > 0$. Če $A$ ni pozitivna matrika,
potem obstaja enotski vektor $x \in \mathcal{H}^n$, tako da je $\langle Ax, x \rangle = - \varepsilon < 0$.
Če vzamemo sedaj $r < \varepsilon$, potem je $\langle (r \cdot I_n + A)x, x \rangle = r - \varepsilon < 0$,
zato tudi $r \cdot I_n + A$ ni pozitivna, protislovje. 

Velja pa tudi obratno: vsak abstrakten operatorski sistem je izomorfen konkretnemu operatorskemu sistemu, 
ki ima na zgoraj opisani način dano strukturo abstraktnega operatorskega sistema.
To je vsebina naslednjega izreka.
...
Za dokaz tega izreka bomo potrebovali analog trditve \ref{trd:1.7}.
Tudi če je $\mathcal{S}$ abstrakten operatorski prostor, 
lahko poljubni preslikavi $\varphi \in \mathcal{L}(\mathcal{S}, M_n (\C))$
...
Na enak način kot prej (glej odstavek pred trditvijo \ref{trd:1.7}) lahko dokažemo, da sta preslikavi 
$\varphi \mapsto s_{\varphi}$ in $s \mapsto \varphi_s$ druga drugi inverzni.
Naslednjo trditev dokažemo na enak način kot \ref{trd:1.7}, zato dokaz izpustimo.
...
\begin{dokaz}[Dokaz izreka \ref{izr:choi-effros}]
    ...
    tako da $s(A_0) \not\geq 0$.
    Po Hahn-Banachovem separacijskem izreku (DOPOLNI) obstaja realen linearen funkcional
    $s_0: M_n (\mathcal{S}) \to \R$, ki loči zaprto konveksno množico $\mathcal{C}_n$ od $A_0$, torej obstajata
    $$ s_0(A_0) < \sup \{ s_0(B)\ |\ B \in \mathcal{C}_n\}.$$
    Najprej opazimo, da je $s_0 (B) \geq 0$ za vsak $B \in \mathcal{C}_n$. Če bi namreč obstajal $B \in \mathcal{C}_n$,
    tako da je $s_0 (B) < 0$, potem je $\frac{s_0 (A_0)}{s_0(B)} > 0$ in zato $\frac{s_0 (A_0)}{s_0(B)} B \in \mathcal{C}_n$.
    To pa bi pomenilo, da je $$s_0 (A_0) < s_0 \left(\frac{s_0 (A_0)}{s_0(B)} B\right) = s_0 (A_0),$$ kar vodi v protislovje.
    ...
    $$V W = U^* \begin{bmatrix}
        ...
        0 & 0
    \end{bmatrix} U = Q.$$
    Sedaj definiramo $\varphi := V^* \varphi_s V \in \EPP (\mathcal{S}, M_k (\C))$, tako da je 
...
To vprašanje lahko ekvivalentno prevedemo na (konkretna) operatorske sisteme. $\mathcal{S} \subseteq C^* (\mathcal{S})$. 
Spomnimo, da je izomorfizem v kategoriji $\osys_1$ enotska povsem izometrična bijekcija med operatorskima sistemoma.
...
\begin{definicija}
    ...
    $C^*$-pokritji $\mathcal{S}$ in obstaja takšen $*$-homomorfizem $\pi: \mathfrak{B}_1 \to \mathfrak{B}_2$,
...
\begin{definicija}
    \emph{$C^*$-ogrinjača} operatorske algebre $\mathcal{S}$ definiramo kot končni objekt $(i, C_{\env} ^* (\mathcal{S}))$ v kategoriji $C^*$-pokritij $\mathcal{S}$:
    ...
    \begin{equation*}
        ...
        % top arrow (unlabeled)
        \draw[->]  (S) -- node[left] {$j$} (A);
        
        % left diagonal arrow
        \draw[->] (S) -- node[left] {$i$} (Q);
...
V tem podrazdelku dokažemo, da $C^*$-ogrinjača (in posledično ideal Šilova) res obstaja za vsak operatorski sistem $\mathcal{S} \subseteq C^*(\mathcal{S})$.
Pri tem sledimo argumentom Dritschell-McCollougha (\cite{dritschel2005}) in Arvesona (\cite{arveson2003}).
...
\begin{definicija}
    ...
    Rečemo tudi, da je $\varphi$ \emph{kompresija} za $\psi$.
...
\begin{definicija}
    ...
    potem je $\psi = \varphi \oplus \psi'$ za neko enotska povsem pozitivna preslikava $\psi'$.
\end{definicija}
...
\begin{trditev}\label{trd:2.2}
    Naj bosta $\mathcal{S}_1 \subseteq C^*(\mathcal{S}_1),  \mathcal{S}_1 \subseteq C^*(\mathcal{S}_1)$ operatorski sistema in $\theta: \mathcal{S}_1 \to \mathcal{S}_2$
...
\begin{dokaz}
    ...
    $$\psi_\lambda: \mathcal{S} \to \mathcal{B}(\mathcal{H}_\infty),\quad \psi_\lambda (a) = \begin{bmatrix}
        ...
        0 & 0
    \end{bmatrix},$$
    torej je $\{\psi_\lambda\}_{\lambda \in \Lambda}$ mreža v $\PP_1 (\mathcal{S}, \mathcal{B}(\mathcal{H}_\infty))$.
    Ker je slednji prostor kompakten, ima mreža neko stekališče $\varphi_\infty \in \PP_1 (\mathcal{S}, \mathcal{B}(\mathcal{H}_\infty))$.
...
Od tod sledi, da je $C^*$-ogrinjača dobro definirana tudi
za abstraktne operatorske sisteme.
...
\section{Nekomutativen rob Šilova}
...
Da bi motivirali definicijo in utemeljili terminologijo, si najprej oglejmo pojem robu Šilova za operatorske sisteme v $C(X)$ in ga nato prevedimo v jezik $C^*$-algeber.
...
Po Stone-Weierstrassovem izreku (DOPOLNI) je to ekvivalentno $C^*(\mathcal{S}) = C(X)$.
...
\begin{definicija}
    ...
    Robni množici $\mathcal{S}$ pravimo \emph{rob Šilova}, če vsebuje vse ostale robne množice.
\end{definicija}
...
\begin{opomba}
    ...
    Teorija uniformiranih algeber je podrobneje motivirana ter obravnavana v (DOPOLNI).
\end{opomba}
...
\begin{zgled}\label{zgl:2.1}
    ...
    \begin{align*}
        ...
        &= 4 (|p'|^2 + |q'|^2) \geq 0.
    \end{align*}
    Po principu maksimum za subharmonične funkcije (DOPOLNI) $|f|^2$ doseže svoj maksimum na $\T$.
    ...
    torej mora biti $\lambda$ element robu Šilova za $\mathcal{S}$.
\end{zgled}
...
V tem poglavju bomo pojem robu Šilova posplošili na
...
\begin{opomba}
    ...
    $*$-homomorfizem (bralec lahko dokaz najde v (DOPOLNI)).    
\end{opomba}
...
na $\mathcal{S}$ izometrija. Če je $r_Y |_{\mathcal{S}}$ izometrija, je 
po opombi \ref{opo:1.1} enotska povsem skrčitev in enako velja tudi za njen inverz $r_Y ^{-1}: r_Y (\mathcal{S}) \to C(X)$, zato je
$r_Y |_\mathcal{S}: \mathcal{S} \to C(Y)$ povsem izometrija.
Slednje pa je ekvivalentno temu, da je $q_{I(Y)} |_{\mathcal{S}}: \mathcal{S} \to \quot{C(X)}{I(Y)}$ povsem izometrija.
...
\begin{zgled}[DOPOLNI] \label{zgl:2.2}
...
\begin{zgled}\label{zgl:2.3}
    Prejšnji zgled lahko posplošimo do neskončno dimenzionalnih matrik nad Hilbertovim prostorom $\ell^2 (\Z_+)$.
    Zanimajo nas torej operatorji $T \in \mathcal{B}(\ell^2 (\Z_+))$, ki imajo glede na standardno bazo $\{e_n\}_{n \in \Z}$ matriko oblike 
    ...
    Dokaze v tem zgledu izpustimo in se zgolj skličemo na knjigo (DOPOLNI), v kateri je ta material podrobneje obravnavan.
    ...
    Sedaj so po izreku (DOPOLNI) operatorji $T \in \mathcal{B}(\ell^2 (\Z_+))$ oblike \eqref{eq:aux2}
    ...
    Označimo $S = T_z \in \mathcal{T} (C(\T))$, ki ustreza operatorju premika v desno na $\ell^2 (\Z_+)$,
    ...
    Izrek (DOPOLNI) nam pove, da je Toeplitzova algebra oblike 
    ...
    Najprej dokažimo, da je $\mathcal{K}(\mathcal{H})$ robni ideal za $\mathcal{T}(C(\T))$.
    ...
    Po zgornjem diagramu je zato tudi $q_{\mathcal{K}(H^2)} |_{\mathcal{T}(C(\T))}: \mathcal{T}(C(\T)) \to \quot{C^*({S})}{\mathcal{K}(H^2)}$
